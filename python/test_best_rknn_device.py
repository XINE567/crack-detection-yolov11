...
print("=" * 50)
print("测试 yolo11.rknn 模型 (RKNN Lite)")
print("=" * 50)
...
# 预处理：正确格式！NHWC RGB [0, 255]，不做 transpose！
img_resized = cv2.resize(img, (640, 640))
rgb = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)
blob = rgb.astype(np.float32)  # 保持 [0, 255]
blob = np.expand_dims(blob, axis=0)  # 变成 (1, 640, 640, 3) NHWC！
...
cv2.imwrite("/userdata/resultb_rknn.jpg", result)
cv2.imwrite("/userdata/resultb_rknn_mask.png", mask_binary)

rknn.release()

print("\n" + "=" * 50)
print("结果已保存:")
print("  - /userdata/resultb_rknn.jpg (可视化)")
print("  - /userdata/resultb_rknn_mask.png (二值化 Mask)")
print("=" * 50)
